...
export class UsersService {
    ...
    try {
      ...
      if (currentRole !== UserRole.OTHER) {
        throw new ForbiddenException(
          'You do not have permission to modify account status ',
        );
      }

      const res = await this.usersRepository.registerAsUser(
        user.user_id,
        UserRole.STUDENT,
      );
      return res;
  ...
  async updatePassword(userId: any, updatePassword: UpdatePasswordDto) {
    try {
      ...
      const success = await this.authRepository.updatePasswordHash(
        userId,
        hashedNewPassword,
      );
  ...
  async deActivateAccount(userid: string) {
    try {
      const res = await this.usersRepository.deActivateAccount(
        userid,
        UserStatus.DELETED,
      );
      return res;
  ...
  async joinOrganization(userId: string, input: JoinOrganizationDto) {
    try {
      const isValid = await this.organizationsService.isValid(
      ...
      if (isAlreadyRequested) {
        throw new ConflictException(
          'You have already submitted a join request to this organization.',
        );
      }

      await this.organizationsService.enterJoinOrganizationRequest(
        userId,
        input.organizationId,
      );
