...
export class OrganizationsController {
    ...
    async createOrganizations(@User() user: IAuthenticatedUser,@Body() input : OrganizationInputDTO){
      return this.organizationsservice.createOrganizations(user ,input);
    }

    @Patch()
    @Roles(UserRole.ORGANIZATION)
    async updateOrganizations(@Organization() organization: IOrganization,@Body() input: OrganizationInputDTO){
      return this.organizationsservice.updateOrganizations(organization,input);
    }

    @Get('/admin/list')
    @Roles(UserRole.ADMIN)
    async getAllOrganizations(@Query('status') status?:string, @Query('isdeleted') isdeleted?:boolean){
      return this.organizationsservice.getAllOrganizations(status,isdeleted);
    }

    @Get('')
    @Roles(UserRole.ADMIN,UserRole.ORGANIZATION)
    async getOranizations(@Organization() organization: IOrganization,@Query('id', ParseUUIDPipe) id?: string){
 ...
 // 1. Fetch all applications waiting for review
  ...
  async getPendingJoinRequests(@Organization() organization: IOrganization) {
    return this.organizationsservice.getPendingJoinRequests(organization.org_id);
  }
  ...
  async handleJoinRequestStatus(
    ...
    return this.organizationsservice.handleJoinRequestStatus(organization.org_id, requestId, input);
  }
