...
export class OrganizationsService {
  ...
  async createOrganizations(
    ...
    try {
      const isTaken = await this.organizationsrepository.checkExisting(
        user.user_id,
        input.name,
      );
      ...
      if (isTaken) {
        throw new ConflictException(
          `The name ${input.name} is already taken by another organization.`,
        );
      }
  ...
  async updateOrganizations(
    ...
    try {
      ...
      const response = await this.organizationsrepository.updateOrganization(
        organization.user_id,
        input.name,
      );
      if (!response.success) {
  ...
  async findOrganizationDetailsByUserId(userId: string) {
    try {
      return await this.organizationsrepository.findOrganizationDetailsByUserId(
        userId,
      );
    } catch (err) {
      return null;
  ...
  async getOrganizations(organization: IOrganization, id?: string) {
    try {
      ...
      const organizationDetails =
        await this.organizationsrepository.getOrganizationDetails(orgId);
  ...
  async updateOrganizationStatus(input: UpdateOrgStatusDto) {
    try {
      const isUpdated =
        await this.organizationsrepository.updateOrganizationStatus(input);
      if (!isUpdated) {
        throw new BadRequestException(
          'Failed to update organization status. Please verify the ID.',
        );
      }
      return {
  ...
  async deleteOrganization(organization: IOrganization) {
    try {
      ...
      throw new BadRequestException(
        'Failed to terminate organization account.',
      );
    }
  }
  ...
  async handleJoinRequestStatus(
    ...
    try {
      ...
      if (input.action === JoinRequestStatus.REJECTED) {
        ...
        await this.organizationsrepository.approveJoinRequestAndCreateMembership(
          requestId,
        );
      }
  ...
  async checkExistingRequest(userId: string, organizationId: string) {
    try {
      return this.organizationsrepository.checkExistingRequest(
        userId,
        organizationId,
      );
    } catch (err) {
  ...
  async enterJoinOrganizationRequest(userId: string, organizationId: string) {
    try {
      return this.organizationsrepository.enterJoinOrganizationRequest(
        userId,
        organizationId,
      );
    } catch (err) {
