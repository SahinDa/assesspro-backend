...
import { CreateOrgOrder, CreateStudentOrder } from "./interface/order.interface";
import { OrganizationTransaction } from "./entities/organizationtransaction.entity";
import { OrgPaymentMethod, OrgTransactionFailureReason, OrgTransactionStatus, PlatformBillingCycle, StudentPaymentMethod, StudentTransactionFailureReason, StudentTransactionStatus } from "src/config/enum";
...
export class PaymentsRepository {
    ...
    async markOrganizationOrderAsSuccess(input: {
        ...
        if (!transaction) {
            throw new Error(`Organization transaction not found for order ID: ${input.gatewayOrderId}`);
        }
    ...
    async markStudentOrderAsSuccess(input: {
        ...
        if (!transaction) {
            throw new Error(`Student transaction not found for order ID: ${input.gatewayOrderId}`);
        }
    ...
    async getOrganizationTransaction(transactionId: string) {
        try {
            return await this.datasource.getRepository(OrganizationTransaction)
                .createQueryBuilder('ot')
                .where('ot.transaction_id = :transactionId', { transactionId })
                .andWhere('ot.status  = : status ', { status: OrgTransactionStatus.Success })
                .getOne();
    ...
    async getUserTransaction(transactionId: string) {
        try {
            return await this.datasource.getRepository(StudentTransaction)
                .createQueryBuilder('st')
                .where('st.transaction_id = :transactionId', { transactionId })
                .andWhere('st.status  = : status ', { status: StudentTransactionStatus.Success })
                .getOne();
    ...
    const repo = this.datasource.getRepository(OrganizationTransaction);
    const transaction = await repo.findOne({ where: { gateway_order_id: input.gatewayOrderId } });
...
try{
  return await this.datasource
    ...
    throw new InternalServerErrorException('Failed to fetch platform transactions');
}
}

async getPlatformTransactionsByOrgId(orgId: string): Promise<OrganizationTransaction[]> {
        try {
            return await this.datasource
                .getRepository(OrganizationTransaction)
                .createQueryBuilder('ot')
            ...
            throw new InternalServerErrorException('Failed to fetch organization platform transactions');
        }
    }
    ...
    async findStudentTransactionsByOrgId(orgId: string): Promise<StudentTransaction[]> {
        return await this.datasource
    ...
    async findStudentTransactionsByUserId(orgId: string, userId: string): Promise<StudentTransaction[]> {
        return await this.datasource
            .getRepository(StudentTransaction)
            .createQueryBuilder('st')
            .where('st.organization_id = :orgId', { orgId })
    ...
    async findSingleStudentTransaction(orgId: string, userId: string, transactionId: string): Promise<StudentTransaction | null> {
        return await this.datasource
            .getRepository(StudentTransaction)
            .createQueryBuilder('st')
            .where('st.organization_id = :orgId', { orgId })
