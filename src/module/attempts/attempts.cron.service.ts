    ...
    try {
      ...
      do {
        ...
        const expiredAttempts =
          await this.attemptsrepository.getExpiredAndAbandonedAttempts(
            batchSize,
          );
        ...
        this.logger.log(
          `CRON: Auto-finalizing a batch of ${fetchedCount} expired exams.`,
        );

        // 2. Map directly to your dedicated backend grading function
        const processingPromises = expiredAttempts.map(async (attempt) => {
          try {
            await this.attemptsservices.autoFinalizeExpiredAttempt(
              attempt.user_id,
              attempt.attempt_id,
            );
            this.logger.log(
              `CRON: Successfully processed attempt ${attempt.attempt_id}`,
            );
          } catch (error) {
            this.logger.error(
              `CRON: Error processing attempt ${attempt.attempt_id}: ${error.message}`,
            );
          }
        });
      ...
      this.logger.error(
        `CRON: Fatal exception encountered during execution: ${err.message}`,
      );
    }
  }
}
